...
#include "ntshim.h"  // ARM port: neutralize MSVC keywords (__int64, __stdcall, ...) before any use
...
typedef unsigned short WCHAR;    // ARM port: NT WCHAR is 16-bit UTF-16; GNU wchar_t is 32-bit, so use unsigned short directly (with -fshort-wchar for L"" literals)
...
//
// ARM port: GCC synthesizes 64-bit (long long) arithmetic, so the plain-C
// expressions (as used for MIDL/Alpha/PPC) are correct and avoid the x86
// inline-assembler branch.
//
...
//
// ARM port: EXCEPTION_DISPOSITION normally arrives via excpt.h (SEH), which the
// loader does not pull in. Provide the canonical enum so PEXCEPTION_ROUTINE
// type-checks; the loader never raises/handles structured exceptions.
//
