...
//
// ARM port: PE machine 0x01c0 (IMAGE_FILE_MACHINE_ARM, little-endian).
//
// KSEG0_BASE is how the loader forms a usable address from a physical page:
// BlMemoryInitialize does `BlHeapFree = KSEG0_BASE | (page << PAGE_SHIFT)`. On
// x86 the loader runs with paging on and 0x80000000 is mapped; on MIPS/Alpha
// 0x80000000 is the architectural unmapped-cached window that the CPU translates
// to physical with no page tables. ARM has no such window, and our loader runs
// with the MMU OFF (start.S sets up no page tables), so virtual == physical and
// KSEG0_BASE must be 0 - otherwise the first heap access faults silently. The
// 0x80000000 kernel virtual base is a separate, later concern (the TARGET_IMAGE
// PE base), revisited when the MMU comes up for kernel handoff.
//

#define TARGET_IMAGE ARM_IMAGE
#define KSEG0_BASE 0
