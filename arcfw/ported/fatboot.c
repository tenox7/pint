...
// ARM port: copy byte-by-byte through volatile UCHAR pointers. The originals do a
// single `*(UCHAR2/4 *)dst = *(UNALIGNED UCHAR2/4 *)src`, which GCC compiles to a
// 16/32-bit load from `src` - and `src` is an unaligned packed on-disk field. On
// Cortex-A7 that alignment-faults (and adjacent ones fuse into a faulting LDRD/LDM)
// regardless of SCTLR.A. Volatile byte copies emit only LDRB/STRB, which have no
// alignment requirement and cannot be fused - the same fix used for the MBR reader.
#define CopyUchar1(Dst,Src) {                                            \
    ((volatile UCHAR *)(Dst))[0] = ((const volatile UCHAR *)(Src))[0];   \
    }

#define CopyUchar2(Dst,Src) {                                            \
    ((volatile UCHAR *)(Dst))[0] = ((const volatile UCHAR *)(Src))[0];   \
    ((volatile UCHAR *)(Dst))[1] = ((const volatile UCHAR *)(Src))[1];   \
    }

#define CopyUchar4(Dst,Src) {                                            \
    ((volatile UCHAR *)(Dst))[0] = ((const volatile UCHAR *)(Src))[0];   \
    ((volatile UCHAR *)(Dst))[1] = ((const volatile UCHAR *)(Src))[1];   \
    ((volatile UCHAR *)(Dst))[2] = ((const volatile UCHAR *)(Src))[2];   \
    ((volatile UCHAR *)(Dst))[3] = ((const volatile UCHAR *)(Src))[3];   \
    }
