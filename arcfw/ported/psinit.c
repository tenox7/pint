...

{
    ...
    KiEmit("PS0: enter PspInitPhase0\n");
    ...
    KiEmit("PS0: locks+resource init OK; copying loaded module list\n");
    ...
    while (NextEntry != &LoaderBlock->LoadOrderListHead) {
        ...
        if (DataTableEntry1 == NULL) {
            KiEmit("PS0: FAIL ExAllocatePool module entry == NULL\n");
            return FALSE;
        }
    ...
    KiEmit("PS0: module list copied; ObCreateObjectType(Process)\n");
    ...
    if ( !NT_SUCCESS(ObCreateObjectType(&NameString,
        ...
        KiEmit("PS0: FAIL ObCreateObjectType(Process)\n");
        return FALSE;
    }

    KiEmit("PS0: ObCreateObjectType(Thread)\n");
    ...
    if ( !NT_SUCCESS(ObCreateObjectType(&NameString,
        ...
        KiEmit("PS0: FAIL ObCreateObjectType(Thread)\n");
        return FALSE;
    }
    ...
    KiEmit("PS0: ExCreateHandleTable (Cid)\n");
    ...
    if ( ! PspCidTable ) {
        KiEmit("PS0: FAIL ExCreateHandleTable == NULL\n");
        return FALSE;
    }
    ...
    KiEmit("PS0: PspCreateProcess(initial system process)\n");
    {
    NTSTATUS _st = PspCreateProcess(
    ...
    if ( !NT_SUCCESS(_st) ) {
        KiEmit("PS0: FAIL PspCreateProcess st=");
        KiEmitHex((unsigned long)_st);
        KiEmit("\n");
        return FALSE;
    }
    }
    KiEmit("PS0: ObReferenceObjectByHandle(process)\n");
    if ( !NT_SUCCESS(ObReferenceObjectByHandle(
        ...
        KiEmit("PS0: FAIL ObReferenceObjectByHandle(process)\n");
        return FALSE;
    }
    ...
    KiEmit("PS0: strcpy ImageFileName OK; PsCreateSystemThread(Phase1)\n");
    {
    NTSTATUS _st = PsCreateSystemThread(
                    &ThreadHandle,
                    THREAD_ALL_ACCESS,
                    &ObjectAttributes,
                    0L,
    ...
    if ( !NT_SUCCESS(_st) ) {
        KiEmit("PS0: FAIL PsCreateSystemThread st=");
        KiEmitHex((unsigned long)_st);
        KiEmit("\n");
        return FALSE;
    }
    ...
    KiEmit("PS0: ObReferenceObjectByHandle(thread)\n");
    if ( !NT_SUCCESS(ObReferenceObjectByHandle(
                        ThreadHandle,
                        0L,
                        PsThreadType,
        ...
        KiEmit("PS0: FAIL ObReferenceObjectByHandle(thread)\n");
        return FALSE;
    }

    KiEmit("PS0: ZwClose(thread handle); PspInitPhase0 -> TRUE\n");
